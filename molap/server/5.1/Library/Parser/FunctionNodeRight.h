...
public:
	...
	FunctionNodeRight(const string& name, vector<Node*> *params) :
			...
			if (l.type == Node::NODE_STRING && l.stringValue.size() > 0) {
				if (right) {
					...
					if (r.type == Node::NODE_NUMERIC) {
						int32_t num = (int32_t)r.doubleValue;

						RuleValueType result;
						result.type = Node::NODE_STRING;
						result.stringValue = UTF8Comparer::right(l.stringValue, num);
					...
					RuleValueType result;
					result.type = Node::NODE_STRING;
					result.stringValue = UTF8Comparer::right(l.stringValue, 1);
					return result;
				}
			}
		}
