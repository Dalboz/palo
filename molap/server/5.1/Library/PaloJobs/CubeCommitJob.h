...
#ifndef PALO_JOBS_CUBE_COMMIT_JOB_H
#define PALO_JOBS_CUBE_COMMIT_JOB_H 1
...
class SERVER_CLASS CubeCommitJob : public DirectPaloJob {
public:
	...
	static PaloJob* create(PaloJobRequest* jobRequest) {
		return new CubeCommitJob(jobRequest);
	}
...
public:
	...
	CubeCommitJob(PaloJobRequest* jobRequest) :
	...
	void compute() {
		...
		for (int commitTry = 0; commitTry < Commitable::COMMIT_REPEATS; commitTry++) {
			...
			cube->commitCube(jobRequest->lock, user);
			ret = server->commit();
		...
		if (!ret) {
			throw CommitException(ErrorException::ERROR_COMMIT_CANTCOMMIT, "Can't commit changes to cube.");
		}
