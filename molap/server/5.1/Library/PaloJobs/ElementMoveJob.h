...
#ifndef PALO_JOBS_ELEMENT_MOVE_JOB_H
#define PALO_JOBS_ELEMENT_MOVE_JOB_H 1
...
class SERVER_CLASS ElementMoveJob : public DirectPaloJob {
public:
	...
	static PaloJob* create(PaloJobRequest* jobRequest) {
		return new ElementMoveJob(jobRequest);
	}
...
public:
	...
	ElementMoveJob(PaloJobRequest* jobRequest) :
	...
	void compute() {
		...
		for (int commitTry = 0; commitTry < Commitable::COMMIT_REPEATS; commitTry++) {
			...
			if (dimension->getDimensionType() == Dimension::VIRTUAL && dimension->getName() == SystemDatabase::NAME_LICENSE_DIMENSION) {
				ret = true;
				server->moveLicense(jobRequest->element, jobRequest->position);
				break;
			} else {
				findElement(true);

				dimension->moveElement(server, database, element, jobRequest->position, user, true);
