...
public:
	bool check(SubSet::Iterator &elem);
	void finalize(vector<SubElem> &result);
	void processChildren(list<SortElem> &elems);

	SDTreeFullSortingFilter(const SortingFilter&, AbstractComparison& comp);
...
public:
	...
	TreeFullSortingFilter(const SortingFilter&, AbstractComparison& comp);
