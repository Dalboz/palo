...
#ifndef LEGACY_SERVER_CUBE_UNLOAD_LEGACY_HANDLER
#define LEGACY_SERVER_CUBE_UNLOAD_LEGACY_HANDLER 1
...
namespace palo {
  ...
  class SERVER_CLASS CubeUnloadLegacyHandler : public LegacyRequestHandler {
  public:
    CubeUnloadLegacyHandler (Server * server) 
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::cube_unload");

      if (arguments->size() != 3) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database and cube as arguments for cube_unload",
