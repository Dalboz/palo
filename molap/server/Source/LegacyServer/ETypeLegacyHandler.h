...
#ifndef LEGACY_SERVER_E_TYPE_LEGACY_HANDLER_H
#define LEGACY_SERVER_E_TYPE_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS ETypeLegacyHandler : public LegacyRequestHandler {
  public:
    ETypeLegacyHandler (Server * server) 
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::etype");

      if (arguments->size() != 4) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database, dimension, and element as arguments for etype",
      ...
      return new ArgumentInt32(legacyType(element->getElementType()));
    }
  };
