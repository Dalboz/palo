...
#ifndef LEGACY_SERVER_E_PREV_LEGACY_HANDLER_H
#define LEGACY_SERVER_E_PREV_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS EPrevLegacyHandler : public LegacyRequestHandler {
  public:
    EPrevLegacyHandler (Server * server) 
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::eprev");

      if (arguments->size() != 4) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database, dimension, and element as arguments for eprev",
