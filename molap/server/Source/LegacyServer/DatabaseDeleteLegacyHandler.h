...
#ifndef LEGACY_SERVER_DATABASE_DELETE_LEGACY_HANDLER_H
#define LEGACY_SERVER_DATABASE_DELETE_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS DatabaseDeleteLegacyHandler : public LegacyRequestHandler {
  public:
    DatabaseDeleteLegacyHandler (Server * server)
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::database_delete");

      if (arguments->size() != 2) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database as argument for database_delete",
