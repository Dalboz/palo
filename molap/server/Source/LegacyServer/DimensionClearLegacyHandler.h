...
#ifndef LEGACY_SERVER_DIMENSION_CLEAR_LEGACY_HANDLER_H
#define LEGACY_SERVER_DIMENSION_CLEAR_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS DimensionClearLegacyHandler : public LegacyRequestHandler {
  public:
    DimensionClearLegacyHandler (Server * server)
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::dimension_clear");

      if (arguments->size() != 3) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database and dimension as arguments for dimension_clear",
