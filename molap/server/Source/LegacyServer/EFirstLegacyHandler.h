...
#ifndef LEGACY_SERVER_E_FIRST_LEGACY_HANDLER_H
#define LEGACY_SERVER_E_FIRST_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS EFirstLegacyHandler : public LegacyRequestHandler {
  public:
    EFirstLegacyHandler (Server * server) 
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::efirst");

      if (arguments->size() != 3) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database and dimension as arguments for efirst",
      ...
      if (elements.empty()) {
        throw ParameterException(ErrorException::ERROR_DIMENSION_EMPTY,
                                 "dimension has no visible elements",
                                 "dimension", dimension->getName());
      }

      return generateElementResult(elements[0]);
    }
  };
