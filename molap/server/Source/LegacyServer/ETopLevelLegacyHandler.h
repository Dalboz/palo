...
#ifndef LEGACY_SERVER_E_TOP_LEVEL_LEGACY_HANDLER_H
#define LEGACY_SERVER_E_TOP_LEVEL_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS ETopLevelLegacyHandler : public LegacyRequestHandler {
  public:
    ETopLevelLegacyHandler (Server * server) 
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::etoplevel");

      if (arguments->size() != 3) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database and dimension as arguments for etoplevel",
      ...
      return new ArgumentUInt32(dimension->getLevel());
    }
  };
