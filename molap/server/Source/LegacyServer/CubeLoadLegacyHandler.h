...
#ifndef LEGACY_SERVER_CUBE_LOAD_LEGACY_HANDLER
#define LEGACY_SERVER_CUBE_LOAD_LEGACY_HANDLER 1
...
namespace palo {
  ...
  class SERVER_CLASS CubeLoadLegacyHandler : public LegacyRequestHandler {
  public:
    CubeLoadLegacyHandler (Server * server) 
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::cube_load");

      if (arguments->size() != 3) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database and cube as arguments for cube_load",
                                 "number of arguments", (int) arguments->size() - 1);
      }

      Database * database = findDatabase(arguments, 1, user);
      Cube * cube         = findCube(database, arguments, 2, user, false);
