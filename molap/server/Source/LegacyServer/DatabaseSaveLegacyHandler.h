...
#ifndef LEGACY_SERVER_DATABASE_SAVE_LEGACY_HANDLER_H
#define LEGACY_SERVER_DATABASE_SAVE_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS DatabaseSaveLegacyHandler : public LegacyRequestHandler {
  public:
    DatabaseSaveLegacyHandler (Server * server)
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::database_save");

      if (arguments->size() != 2) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database as argument for database_save",
