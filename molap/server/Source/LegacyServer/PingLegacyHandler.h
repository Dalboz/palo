...
#ifndef LEGACY_SERVER_PING_LEGACY_HANDLER_H
#define LEGACY_SERVER_PING_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS PingLegacyHandler : public LegacyRequestHandler {
  public:
    PingLegacyHandler (Server * server) 
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::ping");

      if (arguments->size() != 1) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting no arguments for ping",
