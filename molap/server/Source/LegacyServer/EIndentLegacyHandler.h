...
#ifndef LEGACY_SERVER_E_INDENT_LEGACY_HANDLER_H
#define LEGACY_SERVER_E_INDENT_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS EIndentLegacyHandler : public LegacyRequestHandler {
  public:
    EIndentLegacyHandler (Server * server) 
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::eindent");

      if (arguments->size() != 4) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database, dimension, and element as arguments for eindent",
      ...
      uint32_t indent       = element->getIndent(dimension);

      return new ArgumentUInt32(indent);
    }
  };
