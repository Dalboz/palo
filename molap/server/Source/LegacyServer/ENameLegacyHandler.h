...
#ifndef LEGACY_SERVER_E_NAME_LEGACY_HANDLER_H
#define LEGACY_SERVER_E_NAME_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS ENameLegacyHandler : public LegacyRequestHandler {
  public:
    ENameLegacyHandler (Server * server) 
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::ename");

      if (arguments->size() != 4) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database, dimension, and position as arguments for ename",
      ...
      uint32_t position     = asUInt32(arguments, 3)->getValue();
      ...
      return generateElementResult(elements[position - 1]);
    }
  };
