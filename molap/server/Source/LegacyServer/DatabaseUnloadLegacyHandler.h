...
#ifndef LEGACY_SERVER_DATABASE_UNLOAD_LEGACY_HANDLER
#define LEGACY_SERVER_DATABASE_UNLOAD_LEGACY_HANDLER 1
...
namespace palo {
  ...
  class SERVER_CLASS DatabaseUnloadLegacyHandler : public LegacyRequestHandler {
  public:
    DatabaseUnloadLegacyHandler (Server * server) 
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::database_unload");

      if (arguments->size() != 2) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database as argument for database_unload",
