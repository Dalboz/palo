...
#ifndef LEGACY_SERVER_E_INDEX_LEGACY_HANDLER_H
#define LEGACY_SERVER_E_INDEX_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS EIndexLegacyHandler : public LegacyRequestHandler {
  public:
    EIndexLegacyHandler (Server * server) 
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::eindex");

      if (arguments->size() != 4) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database, dimension, and element as arguments for eindex",
      ...
      return new ArgumentUInt32((uint32_t)(iter - elements.begin()) + 1);
    }
  };
