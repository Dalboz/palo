...
#ifndef LEGACY_SERVER_E_MOVE_LEGACY_HANDLER_H
#define LEGACY_SERVER_E_MOVE_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS EMoveLegacyHandler : public LegacyRequestHandler {
  public:
    EMoveLegacyHandler (Server * server) 
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::emove");

      if (arguments->size() != 5) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database, dimension, element, and position as arguments for emove",
      ...
      uint32_t position     = asUInt32(arguments, 4)->getValue();
      ...
      dimension->moveElement(element, elements[position]->getPosition(), user);
