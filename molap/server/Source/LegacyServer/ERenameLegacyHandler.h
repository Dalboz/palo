...
#ifndef LEGACY_SERVER_E_RENAME_LEGACY_HANDLER_H
#define LEGACY_SERVER_E_RENAME_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS ERenameLegacyHandler : public LegacyRequestHandler {
  public:
    ERenameLegacyHandler (Server * server) 
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::erename");

      if (arguments->size() != 5) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database, dimension, element, and new name as arguments for erename",
      ...
      const string& name    = asString(arguments, 4)->getValue();

      dimension->changeElementName(element, name, user);
