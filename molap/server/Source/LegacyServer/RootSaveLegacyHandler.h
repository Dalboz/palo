...
#ifndef LEGACY_SERVER_ROOT_SAVE_LEGACY_HANDLER_H
#define LEGACY_SERVER_ROOT_SAVE_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS RootSaveLegacyHandler : public LegacyRequestHandler {
  public:
    RootSaveLegacyHandler (Server * server)
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::root_save");

      if (arguments->size() != 1) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting no arguments for root_save",
