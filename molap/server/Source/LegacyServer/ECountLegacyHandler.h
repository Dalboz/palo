...
#ifndef LEGACY_SERVER_E_COUNT_LEGACY_HANDLER_H
#define LEGACY_SERVER_E_COUNT_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS ECountLegacyHandler : public LegacyRequestHandler {
  public:
    ECountLegacyHandler (Server * server) 
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::ecount");

      if (arguments->size() != 3) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database and dimension as arguments for ecount",
      ...
      return new ArgumentUInt32((uint32_t) dimension->sizeElements());
    }
  };
