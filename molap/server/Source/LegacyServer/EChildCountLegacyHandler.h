...
#ifndef LEGACY_SERVER_E_CHILD_COUNT_LEGACY_HANDLER_H
#define LEGACY_SERVER_E_CHILD_COUNT_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS EChildCountLegacyHandler : public LegacyRequestHandler {
  public:
    EChildCountLegacyHandler (Server * server) 
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::echildcount");

      if (arguments->size() != 4) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database, dimension, and element as arguments for echildcount",
      ...
      return new ArgumentUInt32((uint32_t) childrenList->size());
    }
  };
