...
#ifndef LEGACY_SERVER_E_CHILD_NAME_LEGACY_HANDLER_H
#define LEGACY_SERVER_E_CHILD_NAME_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS EChildNameLegacyHandler : public LegacyRequestHandler {
  public:
    EChildNameLegacyHandler (Server * server) 
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::echildname");

      if (arguments->size() != 5) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database, dimension, element, and number as arguments for echildname",
      ...
      if (offset == 0 || offset > childrenList->size()) {
          throw ParameterException(ErrorException::ERROR_INVALID_POSITION,
                                   "illegal position",
                                   "position", (int) offset);
      }

      return generateElementResult((*childrenList)[offset-1].first);
    }
  };
