...
#ifndef LEGACY_SERVER_DIMENSION_COMMIT_LOG_LEGACY_HANDLER_H
#define LEGACY_SERVER_DIMENSION_COMMIT_LOG_LEGACY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS DimensionCommitLogLegacyHandler : public LegacyRequestHandler {
  public:
    DimensionCommitLogLegacyHandler (Server * server)
  ...
  public:
    LegacyArgument * handleLegacyRequest (const vector<LegacyArgument*>* arguments, User* user, PaloSession*) {
      Statistics::Timer timer("LegacyHandler::dimension_commit_log");

      if (arguments->size() != 3) {
        throw ParameterException(ErrorException::ERROR_INV_ARG_COUNT,
                                 "expecting database and dimension as arguments for dimension_commit_log",
