...
namespace palo {
  void ArgumentError::serialize (StringBuffer& buffer) const {
    uint32_t type   = ntohl(NET_ARG_TYPE_MULTI);
    uint32_t number = ntohl((uint32_t) 1);
    ...
    type           = ntohl(NET_ARG_TYPE_INT);
    int32_t netval = ntohl(errorCode);
    length         = ntohl(sizeof(netval));

    buffer.appendData((void*) &type,   sizeof(type));
    buffer.appendData((void*) &length, sizeof(length));
    buffer.appendData((void*) &netval, sizeof(netval));
  }
}
