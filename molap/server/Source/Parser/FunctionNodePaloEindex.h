////////////////////////////////////////////////////////////////////////////////
/// @brief parser function node palo eindex
///
/// @file
///
...
#ifndef PARSER_FUNCTION_NODE_PALO_EINDEX_H
#define PARSER_FUNCTION_NODE_PALO_EINDEX_H 1
...
namespace palo {
  ...
  class SERVER_CLASS FunctionNodePaloEindex : public FunctionNodePalo {

    public:
      static FunctionNode* createNode (const string& name, vector<Node*> *params) {
        return new FunctionNodePaloEindex(name, params);
      }
      
    public:
      FunctionNodePaloEindex () : FunctionNodePalo() {
      }

      FunctionNodePaloEindex (const string& name,  vector<Node*> *params) : FunctionNodePalo(name, params) {
      }

      Node * clone () {
        FunctionNodePaloEindex * cloned = new FunctionNodePaloEindex(name, cloneParameters());
      ...
      RuleValueType getValue (CellPath* cellPath, User* user, bool* isCachable, set< pair<Rule*, IdentifiersType> >* ruleHistory) {
        ...
        if (valid) {
          ...
          if (element) {
            ...
            result.doubleValue = (iter - elements.begin()) + 1;
