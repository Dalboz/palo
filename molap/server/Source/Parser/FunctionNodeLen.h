...
namespace palo {
  ...
  class SERVER_CLASS FunctionNodeLen : public FunctionNode {
    ...
    public:
      FunctionNodeLen () : FunctionNode() {
      }

      FunctionNodeLen (const string& name,  vector<Node*> *params) : FunctionNode(name, params) {
      }
      ...
      RuleValueType getValue (CellPath* cellPath, User* user, bool* isCachable, set< pair<Rule*, IdentifiersType> >* ruleHistory) {
        ...
        if (valid) {
          ...
          if (value.type == Node::STRING) {
            result.doubleValue = (double) value.stringValue.length();
            return result;
          }          
        }
