////////////////////////////////////////////////////////////////////////////////
/// @brief parser function node palo echild count
///
/// @file
///
...
#ifndef PARSER_FUNCTION_NODE_PALO_ECHILDCOUNT_H
#define PARSER_FUNCTION_NODE_PALO_ECHILDCOUNT_H 1
...
namespace palo {
  ...
  class SERVER_CLASS FunctionNodePaloEchildcount : public FunctionNodePalo {

    public:
      static FunctionNode* createNode (const string& name, vector<Node*> *params) {
        return new FunctionNodePaloEchildcount(name, params);
      }
      
    public:
      FunctionNodePaloEchildcount () : FunctionNodePalo() {
      }

      FunctionNodePaloEchildcount (const string& name,  vector<Node*> *params) : FunctionNodePalo(name, params) {
      }    

      Node * clone () {
        FunctionNodePaloEchildcount * cloned = new FunctionNodePaloEchildcount(name, cloneParameters());
      ...
      RuleValueType getValue (CellPath* cellPath, User* user, bool* isCachable, set< pair<Rule*, IdentifiersType> >* ruleHistory) {
        ...
        if (valid) {
          ...
          if (element) {
            ...
            if (childrenList) {
              result.doubleValue = (double) childrenList->size();
              return result;
            }              
          }
        }
