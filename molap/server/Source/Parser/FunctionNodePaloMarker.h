...
namespace palo {
  ...
  class SERVER_CLASS FunctionNodePaloMarker : public FunctionNodePalo {
    ...
    public:
      FunctionNodePaloMarker () : FunctionNodePalo() {
      }

      FunctionNodePaloMarker (const string& name,  vector<Node*> *params) : FunctionNodePalo(name, params) {
      }    
      ...
      bool validate (Server* server, Database* database, Cube* cube, Node* destination, string& error) {
        ...
        for (size_t i = 0; i < params->size(); i++) {
          ...
          if (i < 2) {
            ...
            if (param->getNodeType() != Node::NODE_STRING_NODE 
                && param->getNodeType() != Node::NODE_VARIABLE_NODE) {
