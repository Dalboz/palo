...
namespace palo {
  ...
  class SERVER_CLASS FunctionNodePaloEischild : public FunctionNodePalo {
    ...
    public:
      FunctionNodePaloEischild () : FunctionNodePalo() {
      }

      FunctionNodePaloEischild (const string& name,  vector<Node*> *params) : FunctionNodePalo(name, params) {
      }
    ...
    public:
      ...
      RuleValueType getValue (CellPath* cellPath, User* user, bool* isCachable, set< pair<Rule*, IdentifiersType> >* ruleHistory) {
        ...
        if (valid) {
          ...
          Element* parent      = getElement(dimension, params->at(2), cellPath, user,isCachable, ruleHistory);   
          Element* child       = getElement(dimension, params->at(3), cellPath, user,isCachable, ruleHistory);   
