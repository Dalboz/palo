...
#ifndef PARSER_FUNCTION_NODE_EQ_H
#define PARSER_FUNCTION_NODE_EQ_H 1
...
namespace palo {
  ...
  class SERVER_CLASS FunctionNodeEq : public FunctionNodeComparison {
    public:
      static FunctionNode* createNode (const string& name, vector<Node*> *params) {
        return new FunctionNodeEq(name, params, false);
      }

      static FunctionNode* createNodeInfix (const string& name, vector<Node*> *params) {
        return new FunctionNodeEq(name, params, true);
      }

    public:
      FunctionNodeEq () : FunctionNodeComparison() {
      }

      FunctionNodeEq (const string& name,  vector<Node*> *params, bool infix) 
        : FunctionNodeComparison(name, params, infix) {
      }

      Node * clone () {
        FunctionNodeEq * cloned = new FunctionNodeEq(name, cloneParameters(), this->infix);
