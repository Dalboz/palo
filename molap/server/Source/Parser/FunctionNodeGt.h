...
#ifndef PARSER_FUNCTION_NODE_GT_H
#define PARSER_FUNCTION_NODE_GT_H 1
...
namespace palo {
  ...
  class SERVER_CLASS FunctionNodeGt : public FunctionNodeComparison {
      
    public:
      static FunctionNode* createNode (const string& name, vector<Node*> *params) {
        return new FunctionNodeGt(name, params, false);
      }

      static FunctionNode* createNodeInfix (const string& name, vector<Node*> *params) {
        return new FunctionNodeGt(name, params, true);
      }

    public:
      FunctionNodeGt () : FunctionNodeComparison() {
      }

      FunctionNodeGt (const string& name,  vector<Node*> *params, bool infix) : FunctionNodeComparison(name, params, infix) {
      }

      Node * clone () {
        FunctionNodeGt * cloned = new FunctionNodeGt(name, cloneParameters(), this->infix);
