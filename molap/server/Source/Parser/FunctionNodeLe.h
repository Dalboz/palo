...
#ifndef PARSER_FUNCTION_NODE_LE_H
#define PARSER_FUNCTION_NODE_LE_H 1
...
namespace palo {
  ...
  class SERVER_CLASS FunctionNodeLe : public FunctionNodeComparison {
      
    public:
      static FunctionNode* createNode (const string& name, vector<Node*> *params) {
        return new FunctionNodeLe(name, params, false);
      }

      static FunctionNode* createNodeInfix (const string& name, vector<Node*> *params) {
        return new FunctionNodeLe(name, params, true);
      }

    public:
      FunctionNodeLe () : FunctionNodeComparison() {
      }

      FunctionNodeLe (const string& name,  vector<Node*> *params, bool infix) : FunctionNodeComparison(name, params, infix) {
      }

      Node * clone () {
        FunctionNodeLe * cloned = new FunctionNodeLe(name, cloneParameters(), this->infix);
