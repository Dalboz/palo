////////////////////////////////////////////////////////////////////////////////
/// @brief parser function node power
///
/// @file
///
...
#ifndef PARSER_FUNCTION_NODE_POWER_H
#define PARSER_FUNCTION_NODE_POWER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS FunctionNodePower : public FunctionNodeSimple {
    public:
      static FunctionNode* createNode (const string& name, vector<Node*> *params) {
        return new FunctionNodePower(name, params);
      }

    public:
      FunctionNodePower () : FunctionNodeSimple() {
      }

      FunctionNodePower (const string& name,  vector<Node*> *params) : FunctionNodeSimple(name, params) {
      }

      Node * clone () {
        FunctionNodePower * cloned = new FunctionNodePower(name, cloneParameters());
      ...
      RuleValueType getValue (CellPath* cellPath, User* user, bool* isCachable, set< pair<Rule*, IdentifiersType> >* ruleHistory) {
        ...
        if (valid) {
          ...
          if (l.type == Node::NUMERIC && r.type == Node::NUMERIC) {
            result.doubleValue = pow(l.doubleValue, r.doubleValue);
