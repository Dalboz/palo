////////////////////////////////////////////////////////////////////////////////
/// @brief parser function node palo ecount
///
/// @file
///
...
#ifndef PARSER_FUNCTION_NODE_PALO_ECOUNT_H
#define PARSER_FUNCTION_NODE_PALO_ECOUNT_H 1
...
namespace palo {
  ...
  class SERVER_CLASS FunctionNodePaloEcount : public FunctionNodePalo {

    public:
      static FunctionNode* createNode (const string& name, vector<Node*> *params) {
        return new FunctionNodePaloEcount(name, params);
      }
      
    public:
      FunctionNodePaloEcount () : FunctionNodePalo() {
      }

      FunctionNodePaloEcount (const string& name,  vector<Node*> *params) : FunctionNodePalo(name, params) {
      }

      Node * clone () {
        FunctionNodePaloEcount * cloned = new FunctionNodePaloEcount(name, cloneParameters());
    ...
    public:
      ...
      RuleValueType getValue (CellPath* cellPath, User* user, bool* isCachable, set< pair<Rule*, IdentifiersType> >* ruleHistory) {
        ...
        if (valid) {
          ...
          if (dimension) {
            result.doubleValue = (double) dimension->sizeElements();
            return result;              
          }
        }
