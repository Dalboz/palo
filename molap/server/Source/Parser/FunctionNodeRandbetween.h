...
namespace palo {
  ...
  class SERVER_CLASS FunctionNodeRandbetween : public FunctionNodeSimple {
    ...
    public:
      FunctionNodeRandbetween () : FunctionNodeSimple() {
      }

      FunctionNodeRandbetween (const string& name,  vector<Node*> *params) : FunctionNodeSimple(name, params) {
      }
    ...
    public:
      RuleValueType getValue (CellPath* cellPath, User* user, bool* isCachable, set< pair<Rule*, IdentifiersType> >* ruleHistory) {
        ...
        if (valid) {
          ...
          if (l.type == Node::NUMERIC && r.type == Node::NUMERIC && l.doubleValue < r.doubleValue) {            
