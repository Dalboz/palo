...
#ifndef PARSER_FUNCTION_NODE_NE_H
#define PARSER_FUNCTION_NODE_NE_H 1
...
namespace palo {
  ...
  class SERVER_CLASS FunctionNodeNe : public FunctionNodeComparison {
    public:
      static FunctionNode* createNode (const string& name, vector<Node*> *params) {
        return new FunctionNodeNe(name, params, false);
      }

      static FunctionNode* createNodeInfix (const string& name, vector<Node*> *params) {
        return new FunctionNodeNe(name, params, true);
      }

    public:
      FunctionNodeNe () : FunctionNodeComparison() {
      }

      FunctionNodeNe (const string& name,  vector<Node*> *params, bool infix)
        : FunctionNodeComparison(name, params, infix) {
      }

      Node * clone () {
        FunctionNodeNe * cloned = new FunctionNodeNe(name, cloneParameters(), this->infix);
