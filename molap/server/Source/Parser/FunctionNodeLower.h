////////////////////////////////////////////////////////////////////////////////
/// @brief parser function node lower
///
/// @file
///
...
#ifndef PARSER_FUNCTION_NODE_LOWER_H
#define PARSER_FUNCTION_NODE_LOWER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS FunctionNodeLower : public FunctionNode {
      
    public:
      static FunctionNode* createNode (const string& name, vector<Node*> *params) {
        return new FunctionNodeLower(name, params);
      }

    public:
      FunctionNodeLower () : FunctionNode() {
      }

      FunctionNodeLower (const string& name,  vector<Node*> *params) : FunctionNode(name, params) {
      }

      Node * clone () {
        FunctionNodeLower * cloned = new FunctionNodeLower(name, cloneParameters());
    ...
    public:
      ...
      map<Element*, string> computeDimensionTransformations (Cube* cube) {
        ...
        for (map<Element*, string>::iterator i = elements.begin();
             i != elements.end();
             i++) {
          i->second = StringUtils::tolower(i->second);
        }
      ...
      RuleValueType getValue (CellPath* cellPath,
        ...
        if (valid) {
          ...
          if (value.type == Node::STRING) {            
            result.stringValue = StringUtils::tolower(value.stringValue);
            return result;
          }          
        }
