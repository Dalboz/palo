////////////////////////////////////////////////////////////////////////////////
/// @brief parser function node palo eindent
///
/// @file
///
...
#ifndef PARSER_FUNCTION_NODE_PALO_EINDENT_H
#define PARSER_FUNCTION_NODE_PALO_EINDENT_H 1
...
namespace palo {
  ...
  class SERVER_CLASS FunctionNodePaloEindent : public FunctionNodePalo {
    public:
      static FunctionNode* createNode (const string& name, vector<Node*> *params) {
        return new FunctionNodePaloEindent(name, params);
      }
      
    public:
      FunctionNodePaloEindent () : FunctionNodePalo() {
      }

      FunctionNodePaloEindent (const string& name,  vector<Node*> *params) : FunctionNodePalo(name, params) {
      }

      Node * clone () {
        FunctionNodePaloEindent * cloned = new FunctionNodePaloEindent(name, cloneParameters());
    ...
    public:
      ...
      RuleValueType getValue (CellPath* cellPath, User* user, bool* isCachable, set< pair<Rule*, IdentifiersType> >* ruleHistory) {
        ...
        if (valid) {
          ...
          if (element) {
            result.doubleValue = (double) element->getIndent(dimension);
