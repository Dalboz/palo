////////////////////////////////////////////////////////////////////////////////
/// @brief parser function node code
///
/// @file
///
...
#ifndef PARSER_FUNCTION_NODE_CODE_H
#define PARSER_FUNCTION_NODE_CODE_H 1
...
namespace palo {
  ...
  class SERVER_CLASS FunctionNodeCode : public FunctionNode {
      
    public:
      static FunctionNode* createNode (const string& name, vector<Node*> *params) {
        return new FunctionNodeCode(name, params);
      }

    public:
      FunctionNodeCode () : FunctionNode() {
      }

      FunctionNodeCode (const string& name,  vector<Node*> *params) : FunctionNode(name, params) {
      }

      Node * clone () {
        FunctionNodeCode * cloned = new FunctionNodeCode(name, cloneParameters());
        ...
        if (valid) {
          ...
          if (value.type == Node::STRING) {
            if (value.stringValue.length() > 0) {              
              result.doubleValue = (double) value.stringValue[0];
              return result;
            }
          }          
        }
