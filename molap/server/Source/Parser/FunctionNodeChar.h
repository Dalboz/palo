////////////////////////////////////////////////////////////////////////////////
/// @brief parser function node char
///
/// @file
///
...
#ifndef PARSER_FUNCTION_NODE_CHAR_H
#define PARSER_FUNCTION_NODE_CHAR_H 1
...
namespace palo {
  ...
  class SERVER_CLASS FunctionNodeChar : public FunctionNode {
    public:
      static FunctionNode* createNode (const string& name, vector<Node*> *params) {
        return new FunctionNodeChar(name, params);
      }

    public:
      FunctionNodeChar () : FunctionNode() {
      }

      FunctionNodeChar (const string& name,  vector<Node*> *params) : FunctionNode(name, params) {
      }

      Node * clone () {
        FunctionNodeChar * cloned = new FunctionNodeChar(name, cloneParameters());
    ...
    public:
      ...
      RuleValueType getValue (CellPath* cellPath, User* user, bool* isCachable, set< pair<Rule*, IdentifiersType> >* ruleHistory) {
        ...
        if (valid) {
          ...
          if (value.type == Node::NUMERIC) {
            string s(1, (char) value.doubleValue);
