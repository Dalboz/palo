...
namespace palo {
  ...
  public:
    static const uint32_t CUBE_TYPE = 3;
  ...
  public:
    ...
    AttributesCube (IdentifierType identifier,
                const string& name,
                Database* database,
  ...
  private:
    ...
    void checkPathAccessRight (User* user, const CellPath* cellPath, RightsType minimumRight) {
      if (user != 0 && user->getRoleDimensionRight() < minimumRight) {
        throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED,
                                 "insufficient access rights for attributes cube",
    ...
    RightsType getMinimumAccessRight (User* user) {
      ...
      return user->getRoleDimensionRight();
    } 
