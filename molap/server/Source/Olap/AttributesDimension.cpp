////////////////////////////////////////////////////////////////////////////////
/// @brief palo attibutes dimension
///
/// @file
///
...
namespace palo {
  ...
  NormalDimension* AttributesDimension::getNormalDimension () {
    if (normalDimension) {
      return normalDimension;
    }
    
    const string str = name.substr(PREFIX_ATTRIBUTE_DIMENSION.length(), name.length() 
          - PREFIX_ATTRIBUTE_DIMENSION.length() - SUFFIX_ATTRIBUTE_DIMENSION.length());
                         
    Dimension* d = database->lookupDimensionByName(str);
    if (d && d->getType() == NORMAL) {
      normalDimension = dynamic_cast<NormalDimension*>(d);
      return normalDimension;
    }    
  ...
  void AttributesDimension::saveDimensionType (FileWriter* file) {
