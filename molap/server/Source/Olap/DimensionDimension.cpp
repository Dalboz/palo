////////////////////////////////////////////////////////////////////////////////
/// @brief palo dimension dimension
///
/// @file
///
...
namespace palo {
  ...
  void DimensionDimension::saveDimensionType (FileWriter* file) {
  ...
  Element* DimensionDimension::addElement (const string& name, Element::ElementType elementType, User* user, bool useJournal) {
    if (user) {
  ...
  void DimensionDimension::deleteElement (Element * element, User* user, bool useJournal) {
    if (user) {
      throw ParameterException(ErrorException::ERROR_DIMENSION_UNCHANGABLE,
                                 "dimension is not changable",
                                 "user", (int) user->getIdentifier());
  ...
  void DimensionDimension::changeElementName (Element * element, const string& name, User* user) {
    if (user) {
      throw ParameterException(ErrorException::ERROR_DIMENSION_UNCHANGABLE,
                                 "dimension is not changable",
                                 "user", (int) user->getIdentifier());
  ...
  void DimensionDimension::changeElementType (Element * element, Element::ElementType elementType, User* user,
  ...
  void DimensionDimension::notifyAddDimension () {
  ...
  void DimensionDimension::beforeRemoveDimension () {
  ...
  void DimensionDimension::notifyRenameDimension (const string& oldName) {
    // rename attribute dimension and cube
    AttributedDimension::notifyRenameDimension(database, name, oldName);    
  }


  AttributesDimension* DimensionDimension::getAttributesDimension () {    
    return AttributedDimension::getAttributesDimension(database, name);    
  }
    

  AttributesCube* DimensionDimension::getAttributesCube () {
    return AttributedDimension::getAttributesCube(database, name);
  }

  void DimensionDimension::checkElements () {
    
    vector<Element*> elements = getElements(0);
    for (vector<Element*>::iterator i = elements.begin(); i != elements.end(); i++) {
      Dimension* dimension = database->lookupDimensionByName((*i)->getName());
      if (!dimension || dimension->getType() != NORMAL) {
        deleteElement(*i, 0, false);
      }
    }
    
    vector<Dimension*> dimensions = database->getDimensions(0);
