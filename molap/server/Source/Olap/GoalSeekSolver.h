	...
	namespace goalseeksolver 
	{
		...
		public:
			...
			std::vector<int> get_coords(int ind) const {
				std::vector<int> res; res.resize(ec.size());
				int l = (int)ec.size() -1;
				for (int i = 0; i<l; i++) {
					res[l-i] = ind % ec[l-i];
					ind/=ec[l-i];
		...
		public:
			M2D() : colCount(0) {};
			M2D(int c) : colCount(c) {};
			M2D(int c, int r) : colCount(c) {
				row_count(r);	
			};
			int col_count() const {return colCount;};
			void col_count(int c) {
				colCount=c;
				for (int i = 0; i<(int)row.size(); i++)
					row[i].resize(c);
			}
			int row_count() const {return (int)row.size();};
			...
			void append(const RowType& nr) { row.push_back(nr); };

			RowType& operator[](int index) {return row[index];};
			const RowType& operator[](int index) const {return row[index];};
