...
namespace palo {
  bool OptionSpecification::hasSyntaxError (const string& programName) const {
    if (specification.empty()) {
      cerr << programName << ": empty option specifier, "
           << "must be at least 1 character long." << endl;

      return true;
    }
    else if (1 < specification.length() && specification.find_first_of("|?:*+") == string::npos) {
      cerr << programName << ": bad option specifier \"" << specification << ", "
           << "second character must be in the set \"|?:*+\"." << endl;
