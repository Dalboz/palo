...
namespace palo {
  ...
  Options::Options (const string& name, const char * const optv[])
    : programName(name),
      optionControls(OptionSpecification::DEFAULT),
    ...
    if (optv != 0) {
      for (int i = 0;  optv[i] != 0;  i++) {
        ...
        if (optspec->hasSyntaxError(programName)) {
          throw "cannot parse '" + string(optv[i]);
        }
  ...
  string Options::usage (const string& positionals) const {
    ...
    size_t margin = stream.str().size() + 1;
    size_t len = margin;
    ...
    for (vector<OptionSpecification*>::const_iterator iter = specifications.begin();  iter != specifications.end();  iter++) {
      ...
      format(stream, optspec->format(optionControls), margin, len);
    }

    format(stream, positionals, margin, len);
  ...
  int Options::operator() (OptionsIterator & iter, string & optarg) {
    while (true) {
      switch (state) {
      case STATE_ARGUMENTS_EXHAUSTED:           return OPTIONS_END_OPTIONS;
      case STATE_LOAD_ARGUMENT:                 loadArgument(iter);  break;
      case STATE_LOAD_VALUE:                    loadValue(iter);  break;
      case STATE_PROCESS_ARGUMENT:              processArgument(iter);  break;
      case STATE_PROCESS_CURRENT_OPTION:        processCurrentOption();  break;
      case STATE_PROCESS_LONG_OPTION:           processLongOption(iter);  break;
      case STATE_PROCESS_SHORT_OPTION:          processShortOption(iter,true);  break;
      case STATE_PROCESS_SHORT_OPTION_NO_GUESS: processShortOption(iter,false);  break;
      case STATE_RETURN_ERROR:                  return returnError(optarg);  break;
      case STATE_RETURN_LIST_OPTION:            return returnListOption(iter, optarg);
      case STATE_RETURN_POSITIONAL:             return returnPositional(iter, optarg);
      case STATE_RETURN_VALUE:                  return returnValue(optarg);
      }
    }
  }
