...
namespace palo {
  const uint32_t StringUtils::ccitt32[ 256 ] = {
  ...
  static bool simpleMatch (const char* ptr,
                           const char* end,
                           const char* qtr,
                           const char* qnd) {
    for (; qtr < qnd; qtr++) {
  ...
  long StringUtils::stringToInteger (const string& str) {
    if (str.empty()) {
      throw ParameterException(ErrorException::ERROR_CONVERSION_FAILED,
                               "error converting a string to a number, string is empty",
                               "str", str);
    }
  ...
  void StringUtils::decodeBase64 (const string& str, uint8_t * dest, size_t& length) {
    if (str.empty()) {
      length = 0;
      return;
    }
    ...
    for (size_t i = 0;  i < l;  i++) {
      ...
      if (j == 4) {
        ...
        if (p > length) {
          throw ParameterException(ErrorException::ERROR_CONVERSION_FAILED,
                                   "base 64 buffer overflow",
                                   "str", str);
        }

        if (buffer[1] != '=' && buffer[2] != '=') {
          dest[p++] = ((base64ToInt(buffer[1]) & 0x0f) << 4) + (base64ToInt(buffer[2]) >> 2);
        ...
        if (buffer[2] != '=' && buffer[3] != '=') {
          dest[p++] = ((base64ToInt(buffer[2]) & 0x03) << 6) + (base64ToInt(buffer[3]));
  ...
  string StringUtils::escapeXml (const string& str) {
    ...
    bool StringUtils::splitString2Sep (const string& line, vector<vector<string> >* elements, char separator1, char separator2, bool quote) {	
		...
		for (size_t i = 0; i<=line.size(); i++) {
			if ((!quote || q%2==0) && (i==line.size() || line[i]==separator1 || line[i]==separator2))
			{
				if (newline) 
					elements->push_back(vector<string>());
				elements->back().push_back(ss->str());
				newline = i==line.size() || line[i]==separator1;				
				delete ss;	
				if (i==line.size())
					return true;
				q = 0;
  ...
  string StringUtils::getNextElement (string& buffer, size_t& pos, char seperator, bool quote) {
    if (pos >= buffer.size()) {
      return "";
    }
