...
namespace palo {
  ...
  HtmlFormatter::HtmlFormatter (const string& templateName) {
    htmlDocumentation.initialize();

    ifstream file(templateName.c_str());
      
    if (! file) {
      Logger::warning << "could not open documentation file '" << templateName << "'" << endl;
      return;
    }
    ...
    for (vector<string>::const_iterator iter = templateBuffer.begin();  iter != templateBuffer.end();  iter++) {
      ...
      if (s == 0) {
        ...
        if (end != string::npos) {
          ...
          loopVariable = Documentation::trim(loopVariable);
        }
  ...
  void HtmlFormatter::processTemplateLine (Documentation * documentation,
                                           const string& line,
                                           bool withLoop,
                                           size_t maxLoops) {
