...
namespace palo {
  ...
  bool ListenTask::handleRead () {
    ...
    if (connfd == INVALID_SOCKET) {
      Logger::warning << "accept failed in " << __FUNCTION__ << "(" << __FILE__ << "@" << __LINE__ << ")" 
    ...
    if (res != 0 ) {
      ...
      Logger::warning << "setsockopt failed in " << __FUNCTION__ << "(" << __FILE__ << "@" << __LINE__ << ")" 
                      << " with " << errno_socket << " (" << strerror_socket(errno_socket) << ")" << endl;
