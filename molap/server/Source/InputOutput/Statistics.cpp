////////////////////////////////////////////////////////////////////////////////
/// @brief statistics, timer, and counter
///
/// @file
///
...
namespace palo {
  ...
  Statistics::Timer::Timer (const string& name)
    : name (name) {
  ...
  void Statistics::end (Path& path, const string& name) {
    if (! path.isActive) {
      Logger::error << "missing begin timer call for '" 
                    << name << "'" << endl;
    }
    else if (path.activeName != name) {
      Logger::error << "mismatch in timer calls, expecting '" 
                    << path.activeName << "', got '" << name << "'" << endl;
    }
    else {
      if (path.path.empty()) {
        path.isActive   = false;
