...
namespace palo {
  // TODO guess contentType by file extension
  ...
  HttpResponse * HttpFileRequestHandler::handleHttpRequest (const HttpRequest * request) {
    ...
    while (! file.eof()) {
      file.read(c, max);
      response->getBody().appendText(c, file.gcount());
    }
