...
#ifndef HTTP_SERVER_CELL_VALUE_HANDLER_H
#define HTTP_SERVER_CELL_VALUE_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS CellValueHandler : public PaloHttpHandler {
  public:
    CellValueHandler (Server * server) 
  ...
  public:
    HttpResponse * handlePaloRequest (const HttpRequest * request) {
      ...
      Cube::CellValueType value = cube->getCellValue(&cp, &found, user, session, &ruleHistory);
      ...
      return generateCellResponse(cube, &value, found, showRule, showLockInfo, lockInfo);
    }
  };
