...
#ifndef HTTP_SERVER_CELL_AREA_COUNTER_HANDLER_H
#define HTTP_SERVER_CELL_AREA_COUNTER_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS CellAreaCounterHandler : public PaloHttpHandler {
  public:
    CellAreaCounterHandler (Server * server) 
  ...
  public:
    HttpResponse * handlePaloRequest (const HttpRequest * request) {
      ...
      double numBaseCells = cube->getNumAreaBaseCells(&area);
      ...
      response->getBody().appendText(s1.str());
