...
#ifndef HTTP_SERVER_ELEMENT_MOVE_HANDLER_H
#define HTTP_SERVER_ELEMENT_MOVE_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS ElementMoveHandler : public PaloHttpHandler {
  public:
    ElementMoveHandler (Server * server) 
    ...
    HttpResponse * handlePaloRequest (const HttpRequest * request) {      
      ...
      if (posString.empty()) {
        throw ParameterException(ErrorException::ERROR_INVALID_POSITION,
                                 "wrong value for new position",
                                 POSITION, posString);
      }

      PositionType pos = StringUtils::stringToInteger(posString);

      dimension->moveElement(element, pos, user);
