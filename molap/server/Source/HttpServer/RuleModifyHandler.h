...
#ifndef HTTP_SERVER_RULE_MODIFY_HANDLER_H
#define HTTP_SERVER_RULE_MODIFY_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS RuleModifyHandler : public PaloHttpHandler {
    
  public:
    RuleModifyHandler (Server * server) 
    ...
    HttpResponse * handlePaloRequest (const HttpRequest * request) {
      ...
      if (definition != "") {
        ...
        if (r) {
          ...
          Rule* rule = findRule(cube, request, 0);
          ...
          cube->modifyRule(rule, r, external, comment, user);

          if (activate == "0") {
            cube->activateRule(rule, false, user);
          }
          else if (activate == "1") {
            cube->activateRule(rule, true, user);
          }
           
          return generateRuleResponse(rule, useIdentifier);
        }
        else {
