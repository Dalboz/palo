...
#ifndef HTTP_SERVER_CUBE_LOCK_HANDLER_H
#define HTTP_SERVER_CUBE_LOCK_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS CubeLockHandler : public PaloHttpHandler {
    
  public:
    CubeLockHandler (Server * server) 
  ...
  public:
    HttpResponse * handlePaloRequest (const HttpRequest * request) {
      ...
      if (pathIds.empty()) {
        const string& pathNames = request->getValue(NAME_AREA);
        lock = cube->lockCube(&area, pathNames, user);
      }
      else {
        lock = cube->lockCube(&area, pathIds, user);
      }      
  
      return generateLockResponse(server, cube, lock);
    }
  };
