...
#ifndef HTTP_SERVER_CELL_GOALSEEK_HANDLER_H
#define HTTP_SERVER_CELL_GOALSEEK_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS CellGoalSeekHandler : public PaloHttpHandler {
    
  public:
    CellGoalSeekHandler (Server * server) 
  ...
  public:
    HttpResponse * handlePaloRequest (const HttpRequest * request) {
	  ...
	  cube->cellGoalSeek(&cp, user, session, value);      
