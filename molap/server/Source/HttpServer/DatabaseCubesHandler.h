...
#ifndef HTTP_SERVER_DATABASE_CUBES_HANDLER_H
#define HTTP_SERVER_DATABASE_CUBES_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS DatabaseCubesHandler : public PaloHttpHandler {
  public:
    DatabaseCubesHandler (Server * server) 
  ...
  public:
    HttpResponse * handlePaloRequest (const HttpRequest * request) {
      ...
      vector<Cube*> cubes = database->getCubes(user);
