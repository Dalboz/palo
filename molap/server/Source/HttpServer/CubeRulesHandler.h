...
#ifndef HTTP_SERVER_CUBE_RULES_HANDLER_H
#define HTTP_SERVER_CUBE_RULES_HANDLER_H 1
...
namespace palo {
  ...
  class SERVER_CLASS CubeRulesHandler : public PaloHttpHandler {
    
  public:
    CubeRulesHandler (Server * server) 
  ...
  public:
    HttpResponse * handlePaloRequest (const HttpRequest * request) {
      ...
      vector<Rule*> rules = cube->getRules(user);
      ...
      return generateRulesResponse(cube, &rules, useIdentifier);
    }
  };
